...
def _llada_forward_logits(
    ...
    return base.lm_head(outputs.last_hidden_state)
...
def generate_with_lopa(
    model, tokenizer, prompt: str, config: Optional[LopaConfig] = None
) -> str:
    """
    ...
    print(
        f"[LoPA-v2] Starting generation with {config.num_lookahead} lookahead branches"
    )
    ...
    for block_idx in range(prefill_blocks, num_blocks):
        ...
        print(f"[LoPA-v2] Processing block {block_idx} (pos {block_start}:{block_end})")

        # 检查当前 block 是否还有 mask
        current_block = x[:, block_start:block_end]
        if (current_block == config.mask_id).sum() == 0:
            print(f"    [LoPA-v2] Block already complete, skipping")
            continue
        ...
        if config.eos_early_stop:
            ...
            if config.eos_id in generated_part:
                print(f"[LoPA-v2] EOS detected, stopping early")
                break
    ...
    print(f"[LoPA-v2] Generation completed, total tokens: {generated_tokens.shape[1]}")
...
def _lopa_decode_block(
    ...
    for iteration in range(config.max_post_steps + 20):  # 防止无限循环
        ...
        with torch.no_grad():
            logits = _llada_forward_logits(
                model, cur_x, cur_attn_mask, cur_position_ids
            )
...
def _execute_lopa_step(
    ...
    # 创建基础分支 B0
    base_x = cur_x.clone()
    base_x[:, block_start:block_end][high_conf_mask] = x0[high_conf_mask]
    ...
    batch_inputs = torch.stack(
        [branch[:, :current_window_end] for branch in all_branches]
    )

    batch_size = len(all_branches)
    batch_attn_mask = global_attention_mask[
        :, :, :current_window_end, :current_window_end
    ].repeat(batch_size, 1, 1, 1)
    batch_position_ids = global_position_ids[:, :current_window_end].repeat(
        batch_size, 1
    )

    # 批量前向
    with torch.no_grad():
        all_logits = _llada_forward_logits(
            model, batch_inputs, batch_attn_mask, batch_position_ids
        )

    # 计算分支置信度（基于已填充位置的平均概率）
    branch_scores = []
    for i, branch_x in enumerate(all_branches):
        branch_block = branch_x[:, block_start:block_end]
        filled_positions = branch_block != config.mask_id

        if filled_positions.sum() > 0:
            branch_block_logits = all_logits[i : i + 1, block_start:block_end, :]
            probs = F.softmax(branch_block_logits[0], dim=-1)
            filled_probs = torch.gather(
                probs[filled_positions[0]],
                -1,
                branch_block[filled_positions].unsqueeze(-1),
            ).squeeze(-1)
            score = filled_probs.mean().item()
        else:
            score = 0.0
    ...
    print(f"    [LoPA-v2] Step: filled {num_filled}, branches: {branch_info}")
