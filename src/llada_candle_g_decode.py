...
在 CANDLE 的基础上新增三项改进：
1. local candidate score：不再只按局部置信度排序，而是综合
   confidence + consistency + entropy gain；
2. two-level gate：anchor gate + local token gate；
3. directional propagation：默认只向 anchor 右侧传播。
"""
...
@dataclass
class CandleGConfig(BaseCladConfig):
    """CANDLE-G 解码配置。"""
...
def _anchor_gate_status(
    winner_details: Optional[dict], config: CandleGConfig
) -> Tuple[bool, str]:
...
def _build_local_candidate_info(
    pos: int,
    max_prob: float,
    token: int,
    cur_tokens: torch.Tensor,
    cur_ent: torch.Tensor,
    branch_ent: torch.Tensor,
    branch_argmax: torch.Tensor,
    config: CandleGConfig,
) -> dict:
    local_consistency = (
        1.0 if int(branch_argmax[pos].item()) == int(cur_tokens[pos].item()) else 0.0
    )
    log_v = math.log(float(config.vocab_size))
    local_entropy_gain = float(
        ((cur_ent[pos] - branch_ent[pos]) / log_v).clamp(-1.0, 1.0).item()
    )
    local_score = (
        config.local_score_weight_conf * max_prob
        + config.local_score_weight_cons * local_consistency
        + config.local_score_weight_ent * local_entropy_gain
    )
    return {
        "pos": int(pos),
        "token": int(token),
        ...
        "local_entropy_gain": float(local_entropy_gain),
        "local_score": float(local_score),
    }


def _apply_local_leap_tokens_gated(
    x: torch.Tensor,
    ...
    result = {
        ...
        "propagate_right_only": bool(config.propagate_right_only),
        "local_candidates": [],
    }
    ...
    if config.propagate_right_only:
        local_positions = [
            pos for pos in range(anchor_pos + 1, right) if remaining_mask[pos].item()
        ]
    else:
        local_positions = [
            pos for pos in range(left, right) if remaining_mask[pos].item()
        ]

    if not local_positions:
        return result

    branch_block_logits = branch_logits_1[0, block_start:block_end]
    probs = torch.softmax(branch_block_logits, dim=-1)
    max_p, tok = probs.max(dim=-1)
    cur_ent = _per_position_entropy(cur_logits_block)
    branch_ent = _per_position_entropy(branch_block_logits)
    branch_argmax = probs.argmax(dim=-1)

    candidates = [
        _build_local_candidate_info(
            pos,
            float(max_p[pos].item()),
            int(tok[pos].item()),
            cur_tokens,
            cur_ent,
            branch_ent,
    ...
    result["candidate_positions"] = [item["pos"] for item in candidates]
    result["candidate_probs"] = [item["prob"] for item in candidates]

    accepted = 0
    local_candidates_out = []
    for item in candidates:
        if accepted >= config.max_local_accept:
            item["accepted"] = False
            item["rejected_reason"] = "max_local_accept_reached"
            local_candidates_out.append(item)
            continue

        if item["prob"] < config.local_relaxed_threshold:
            item["accepted"] = False
            item["rejected_reason"] = "prob_below_local_threshold"
            local_candidates_out.append(item)
            continue

        if (
            item["local_consistency"] < config.local_token_consistency_floor
            and item["local_entropy_gain"] < config.local_token_entropy_floor
        ):
            item["accepted"] = False
            item["rejected_reason"] = "local_gate_blocked"
            local_candidates_out.append(item)
            continue

        x[0, block_start + item["pos"]] = item["token"]
        result["accepted_positions"].append(item["pos"])
        result["accepted_tokens"].append(item["token"])
        result["accepted_probs"].append(item["prob"])
        item["accepted"] = True
        item["rejected_reason"] = None
        local_candidates_out.append(item)
        accepted += 1

    result["local_candidates"] = local_candidates_out
    result["fired"] = accepted > 0
    return result


def _candleg_lookahead_fill(
    model,
    x: torch.Tensor,
    cur_tokens: torch.Tensor,
    ...
    anchor_gate_passed, anchor_gate_reason = _anchor_gate_status(winner_details, config)
    ...
    if anchor_gate_passed:
        local_info = _apply_local_leap_tokens_gated(
            x,
            best_logits,
            best_pos,
            cur_tokens,
    ...
    print(
        f"    [CANDLE-G] Phase-2: anchor_pos={best_pos} tok={best_tok} "
    ...
    _append_trace(
        trace_out,
        {
            ...
            "anchor_gate_reason": anchor_gate_reason,
    ...
    for _ in range(max_iterations):
        ...
        if history_buffer.has_history:
            ...
            if pos is not None and len(pos) > 0:
                ...
                print(f"    [CANDLE-G] Phase-1 consistency: accepted {len(pos)} tokens")
                accepted = True
                if stats is not None:
        ...
        if (
            ...
            accepted = _candleg_lookahead_fill(
                model,
                x,
                tokens,
                token_probs,
...
def generate_with_candle_g(
    model,
    tokenizer,
    prompt: str,
    config: Optional[CandleGConfig] = None,
    stats_out: Optional[List] = None,
    trace_out: Optional[List[dict]] = None,
) -> Tuple[str, int]:
    """使用 CANDLE-G 策略对 LLaDA2.1-mini 进行解码。"""
    if config is None:
        config = CandleGConfig()
    ...
    print(
        f"[CANDLE-G] top_v={config.top_v} k={config.num_lookahead} "
        ...
        f"right_only={int(config.propagate_right_only)}"
    )
    ...
    for block_idx in range(prefill_blocks, num_blocks):
        ...
        print(f"[CANDLE-G] block {block_idx} ({block_start}:{block_end})")

        if (x[:, block_start:block_end] == config.mask_id).sum() == 0:
            continue

        x = _candleg_decode_block(
            model,
            x,
            block_start,
            block_end,
    ...
    print(f"[CANDLE-G] done, tokens={generated_tokens.shape[1]}")
